...
class GCN_Bert(nn.Module):
    def __init__(self, sequence_length, target_sequence_length,targets_num_max, num_classes, word_embedding_dim, 
                input_x, input_target, input_targets_all, l2_reg_lambda=0.0, num_hidden=100, dropout_keep_prob = 0.5):
        super(GCN_Bert, self).__init__()
        rand_base = 0.01
        #placeholders
        self.input_x = torch.from_numpy(input_x)
        self.input_target = torch.from_numpy(input_target)
        self.input_targets_all = torch.from_numpy(input_targets_all)
        self.dropout_keep_prob = dropout_keep_prob
    
        #embedding layer
        self.embedded_sen = F.dropout(self.input_x, p = self.dropout_keep_prob)  #(?,78,768)
